...
//    abstract <T> String getEntitySetNames(T entity);
    ...
    public Long count(Parameter parameter) throws DaoException {
        ...
        try (PreparedStatement statement = getConnection().prepareStatement(QueryBuilder.getSql(jdbcParameter));
             ResultSet resultSet = setStatementValues(statement, jdbcParameter).executeQuery()) {
            if (resultSet == null || !resultSet.next()) return null;
            return resultSet.getLong(QueryBuilder.FUNCTION_COUNT_RESULT);
        } catch (SQLException e) {
    ...
    @Override
    public <T> List<T> getAll(Parameter parameter)
            throws DaoException {
    ...
    @Override
    public <T> T getFirst(Parameter parameter)
            throws DaoException {
    ...
    abstract String getTableName(String entityClassName);
